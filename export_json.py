"""
export_json.py — Green Gregory Opportunity OS  Phase 7A
Changes vs Phase 6:
  - STALE data_mode: matches exist but their supply is excluded/absent
  - data_mode now driven by supply counts only (not match counts)
  - Matched deals filtered to only those whose supply row is in the included set
  - stale_match_count: matches that were dropped due to excluded/missing supply
  - --clear-stale flag: deletes stale matches from DB before export
  - Full card metadata on supply_opps: description, keywords_hit, source_site
  ...
  - config.json written alongside sources_config.json for scanner/matcher consumption
"""
...
CONFIG_JSON = Path("config.json")          # consumed by scanner.py / matcher.py
...
def save_sources_config(sources: list):
    """Write sources_config.json (read by dashboard) and config.json (read by scanner/matcher)."""
    SOURCES_CFG.write_text(json.dumps(sources, indent=2), encoding="utf-8")
    log.info(f"Saved {len(sources)} sources to {SOURCES_CFG}")

    # Write config.json for scanner.py / matcher.py consumption
    config = {
        "sources": sources,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "note": "Auto-generated by export_json.py — edit via Google Sheet Sources tab or sources_config.json",
    }
    CONFIG_JSON.write_text(json.dumps(config, indent=2), encoding="utf-8")
    log.info(f"Saved config.json ({CONFIG_JSON})")
...
def _source_site(url: str) -> str:
    """Extract readable source site name from URL."""
    if not url:
        return ""
    try:
...
def compute_data_mode(
    seed_s, live_s, manual_s,
    stale_count: int = 0
) -> str:
    """
    data_mode is driven by SUPPLY counts only.
    Match counts are excluded — stale matches do not inflate the mode.

    STALE  — matches exist but no included supply records back them
    EMPTY  — nothing at all
    DEMO   — seed supply only
    LIVE   — live supply only (no seed, no manual)
...
def clear_stale_matches(conn, included_listing_ids: set):
    """
    Delete matches whose listing_id is not in the included supply set.
    Returns count of deleted rows.
    """
    if not included_listing_ids:
        # If nothing is included, delete ALL matches (full purge)
        cur = conn.execute("DELETE FROM matches")
        conn.commit()
        deleted = cur.rowcount
        log.warning(f"clear_stale: no included supply — deleted all {deleted} matches")
        return deleted
    ...
    conn.commit()
    deleted = cur.rowcount
    log.info(f"clear_stale: deleted {deleted} stale matches")
    return deleted
...
def build(
    ...
    # Seed exclusion: exclude rows whose URL contains /seed/
    # (is_seed flag not always set on raw_listings — URL is the canonical check)
    ...
    # Build an id set of included supply records — used to filter matches
    ...
    # ── 1b. Optionally purge stale matches before reading them ────────────────
    if do_clear_stale:
    ...
    # ── 3. Matched deals ──────────────────────────────────────────────────────
    # CRITICAL FIX: Only export matches whose raw_listing is in included_listing_ids.
    # We do this in Python after fetching — avoids complex SQL parameterization
    # and gives us precise stale_match_count.
    ...
    match_rows = conn.execute(f"""
        SELECT m.id, m.score, m.buyer_id, m.buyer_name, m.price_est, m.sell_est,
               ...
               r.price_raw, r.price_est AS supply_price_est,
               r.category, r.end_date, r.is_manual, r.description AS supply_desc,
               r.keywords_hit AS supply_keywords
        FROM matches m
    ...
    for r in match_rows:
        ...
        # KEY FIX: If this match's supply row is not in our included set, it's stale
        ...
        # Classify by supply row properties (not match.is_seed alone)
        is_seed_supply   = _is_seed_url(raw_url)
        is_manual_supply = bool(r["is_manual"])
        is_seed_match    = bool(r["is_seed"])

        # Use supply-row classification as the authoritative source
        ...
        matched_deals.append({
            ...
            "supply_source_site": source_site,
            "supply_url":        url,
    ...
    for r in asm_rows:
        ...
        # Only include assembly opps whose component listings are all in the included set
    ...
    # ── KPIs ──────────────────────────────────────────────────────────────────
    # KPIs reflect included data only
    ...
    # ── Data mode (supply-only logic) ─────────────────────────────────────────
    data_mode = compute_data_mode(
        seed_supply, live_supply, manual_supply,
    ...
    return {
        ...
        # Match counts (included matches only)
...
def main():
    ...
    sources_config = load_sources_config()
    save_sources_config(sources_config)   # writes both sources_config.json and config.json
    ...
    if data["has_stale_data"]:
        log.warning(
            ...
            f"their supply rows are not in the current export. "
            f"Run with --clear-stale to remove them from DB."
        )
