"""
export_json.py — Green Gregory Opportunity OS  Phase 6
Changes vs Phase 4/5:
  - --include-seed-data / --no-seed  flags
  - data_mode: DEMO / LIVE / MANUAL / MIXED / EMPTY
  - Detailed seed/live/manual counts in every section
  - sources_config section exported from Sheet/config
  - last_live_scan_at derived from scraped_at timestamps
  - If no live/manual data and seed excluded → EMPTY mode
"""
...
SOURCES_CFG = Path("sources_config.json")   # written by config_reader / sheet sync

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger("export_json")


# ── Default sources config (used if sources_config.json missing) ──────────────
DEFAULT_SOURCES = [
    {
        "enabled": True,
...
def save_sources_config(sources: list):
    """Write sources_config.json so the dashboard can always see configured sources."""
...
def compute_data_mode(seed_s, live_s, manual_s, seed_m, live_m, manual_m) -> str:
    """
    Compute the data_mode string from counts.
    Priority: EMPTY > DEMO > LIVE > MANUAL > MIXED
    """
    has_live   = (live_s + live_m) > 0
    has_manual = (manual_s + manual_m) > 0
    has_seed   = (seed_s + seed_m) > 0

    if not has_live and not has_manual and not has_seed:
        return "EMPTY"
...
def build(conn, include_seed: bool = True, sources_config: list = None) -> dict:
    ...
    for r in match_rows:
        ...
        is_manual = bool(r["is_manual"])
        if is_seed:   seed_matches   += 1
        elif is_manual: manual_matches += 1
        else:           live_matches   += 1
    ...
    data_mode = compute_data_mode(
        ...
        seed_matches, live_matches, manual_matches
    )
    ...
    return {
        ...
        # Legacy field — keep so old dashboard code doesn't break
...
def main():
    ...
    parser.add_argument("--no-seed",          action="store_true",
                        help="Exclude seed/demo data from output (shows EMPTY if no live/manual data)")
    parser.add_argument("--include-seed-data", dest="include_seed", action="store_true", default=True,
                        help="Include seed data (default: true)")
    parser.add_argument("--exclude-seed-data", dest="include_seed", action="store_false",
                        help="Exclude seed data from output")
    args = parser.parse_args()

    # --no-seed is shorthand for --exclude-seed-data
    ...
    sources_config = load_sources_config()
    # Always write sources_config.json so the dashboard can read it even when run standalone
